...
def init_db():
    ...
    with get_db() as conn:
        ...
        """)
        # Migration: add spotify_url column to existing databases that
        # predate this feature. SQLite has no "ADD COLUMN IF NOT EXISTS",
        # so we check pragma and swallow the duplicate-column error.
        cols = [r["name"] for r in conn.execute("PRAGMA table_info(songs)").fetchall()]
        if "spotify_url" not in cols:
            try:
                conn.execute("ALTER TABLE songs ADD COLUMN spotify_url TEXT")
...
def add_song():
    ...
    with get_db() as conn:
        cur = conn.execute(
            "INSERT INTO songs (title, artist, album_image, preview_url, spotify_url, note) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (title, artist, album_image or None, preview_url or None,
             spotify_url or None, note or None),
        )
        conn.commit()
        new_id = cur.lastrowid
        row = conn.execute(
...
# ---- iTunes song lookup ----------------------------------------------
# The Spotify Web API deprecated preview_url on 2024-11-27, so we use
# Apple's iTunes Search API instead. It's free, needs no authentication,
# returns a 30-second preview URL (.m4a) for nearly every track, plus
# album artwork. No API key required.
@app.route("/api/lookup_song")
def lookup_song():
    ...
    for r in payload.get("results", []):
        # Upgrade the artwork from the 100x100 thumbnail to 600x600.
        artwork = r.get("artworkUrl100") or ""
        if artwork:
            artwork = artwork.replace("100x100bb", "600x600bb")
        title = r.get("trackName") or ""
        artist = r.get("artistName") or ""
        # Build a Spotify search URL so "Listen on Spotify" opens the
        # track inside Spotify. No Spotify API key needed — Spotify's
        # web search handles the lookup.
        spotify_url = ""
        if title and artist:
